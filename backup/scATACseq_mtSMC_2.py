...
# merge and call indel together using outside script. -step 5 , this step finished outside the pipeline
# using indel file to realigne and recal Quality  --step5
...
# The recalibration step is not applied for scATAC-seq , because the sequencing depth is not enough to make a good estimation  
...
#############################################################################################################################
# The recalibration step should be done for merged bam file. 
# Then find a way to call SNP for each sample / cell.  This step should be different for bulk and sc- ATAC data.  
# recalibrator 
# Need to add known SNP sites 
##recal = os.path.join(map_folder, args.sample_name + ".recalibration")
##cmd = "java -Xmx10g -jar " + tools.GATK + " "
##cmd += " -R " + res.ref_genome_fasta + " "
##cmd += " -T BaseRecalibrator "
##cmd += " -knownSites " + res.GIndel + " "  # Now this step is for bulk data 
##cmd += " -I " + realigned_bam + " "
##cmd += " -o " + recal
##pm.run(cmd,recal) 
